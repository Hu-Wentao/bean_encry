...
class AES implements Algorithm {
  ...
  AES(this.key, {this.mode = AESMode.sic, this.padding })
  ...
  @override
  Uint8List decrypt(Encrypted encrypt, {IV iv}) {
    _cipher
      ..reset()
      ..init(false, _buildParams(iv));

    if (padding == null) {
      return _processBlocks(encrypt.bytes);
    }

    return _cipher.process(encrypt.bytes);
  }
