...
class RSA extends AbstractRSA implements Algorithm {
  ...
  Encrypted beanEncrypt(Uint8List bytes, {IV iv}) {
    if (publicKey == null) {
      throw StateError('Can\'t bean_encry without a public key, null given.');
    }
  ...
  @override
  Uint8List decrypt(Encrypted bean_encryed, {IV iv}) {
    if (privateKey == null) {
    ...
    return _cipher.process(bean_encryed.bytes);
  }
}
